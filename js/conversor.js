function verificar_base(str, base) {
  if (base == 2) return str.match(/^[0-1]+$/);
  else if (base == 8) return str.match(/^[0-7]+$/);
  else if (base == 10) return str.match(/^[0-9]+$/);
  else if (base == 16) return str.match(/^[0-9A-F]+$/);
}
...
function a_decimal(base, str_num) {
  ...
  while (str_num != "" && exp < 12) {
    let str_act = str_num.slice(--ult_pos);
